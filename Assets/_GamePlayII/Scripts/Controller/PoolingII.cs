...
public class PoolingII : MonoBehaviour
{
    private static PoolingII instance;
    public static PoolingII Instance { get { return instance; } }
    ...
    public void GenerateObjectPool()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if(ObjectPools[i].parent == null) ObjectPools[i].parent = newParent.transform;
    ...
    {
        int count = 1 + NameObject.GetValues(typeof(PoolingII.NameObject)).Cast<int>().Max();
        for (int i = 0; i < count; i++)
        {
